...
import copy
import pyomo_version.stats.fixed_staff as fs
import pyomo_version.stats.efficiency as ef
import pyomo_version.stats.planet_stats as ps


class cal_process:

    def __init__(self,ovr_workers,ovr_actors,ovr_staff,ovr_prior):
        self.kinds = 7
        self.num_planets = 10
        self.avgpx = 50
        self.elec = 0
    ...
    def fixed_staff(self,planet,pop):
    ...
    def one_planet_cal_rules(self,planet,vec,actors,staff,prior):
        p = copy.deepcopy(planet)
        ...
        stable = self.stability(p,sum(vec),actors,staff)
        addition = 0
        if stable > 50:
            addition = (stable - 50) * 0.006
        else:
            addition = (50 - stable) * 0.01
    ...
    def ovr_cal_rule(self):
        ...
        for i in range(self.num_planets):
            ...
            pr = []
            for j in range(self.kinds):
                w.append(self.prior[i,j])


            self.one_planet_cal_rules(ps.planet[i],w,self.actors[i],self.staff[i],pr)
